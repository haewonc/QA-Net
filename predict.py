...
import torch
from utils.evaluator import shift_cPSNR
from utils.data_loader import ImagesetDataset, ImageSet
from config import Config
from utils.utils import getImageSetDirectories, readBaselineCPSNR, collateFunction
...
def get_sr_and_score(imset, model, min_L=16):
    ...
    if imset.__class__ is ImageSet:
        ...
        lrs, qms, hrs, hr_maps, names = collator([imset])
    elif isinstance(imset, tuple):  # imset is a tuple of batches
        lrs, qms, hrs, hr_maps, names = imset

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    # stacked = torch.cat([lrs, qms], dim=0)
    lrs = lrs.float().to(device)
    qms = qms.float().to(device)
    # if lrs.size(1) > 22:
    #     indices = random.sample(range(lrs.size(1)), 22)
    #     lrs = lrs[:, indices, :, :]
    model.eval()
    with torch.no_grad():
        sr = model(lrs, qms)
...
def load_data(config_file_path, val_proportion=0.10, top_k=9):
    '''
...
#     checkpoint_dir = config["paths"]["checkpoint_dir"]
    ...
    model.load_state_dict(torch.load(checkpoint_file, map_location='cuda:0'))
    return model
...
config = Config()
mod = Model(config)
mod.load_checkpoint("results/saved_models/nir_best.pt")
test_list = getImageSetDirectories("../probav_data/test/", channels="NIR")
test = ImagesetDataset(imset_dir=test_list, patch_size=128, top_k=config.N_lrs)
mod.generate_submission_file(test, "../submission")